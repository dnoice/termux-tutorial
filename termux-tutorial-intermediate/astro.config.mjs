...
export default defineConfig({
	...
	integrations: [
		...
		starlight({
			...
			description: DESCRIPTION,
			tagline: 'Your phone, talking to its own hardware.',
			social: [
				{
					icon: 'github',
					label: 'GitHub',
			...
			sidebar: [
				{
					label: 'The Android Bridge',
					items: [
						{ label: 'Welcome', slug: 'index' },
						{
							// Lesson one is a setup lesson on purpose. Termux:API is two
							// installs that MUST share a signing key, and every later
							// lesson is dead without it — so the failure mode (commands
							// that hang forever instead of erroring) gets diagnosed here,
							// once, rather than five times in five lessons.
							label: 'Wiring Up Termux:API',
							slug: 'bridge/api-setup',
						},
						{
							// Everything the phone will TELL you: battery, network,
							// telephony, sensors, GPS, camera, microphone. Read-only, one
							// command at a time at the prompt — the scripting lesson is
							// what later stitches these together. Also the course's
							// privacy conversation: this is where a learner first points
							// a script at a camera.
							label: 'Reading the Device',
							slug: 'bridge/reading-the-device',
						},
						{
							// The return path, and the reason this course can leave the
							// terminal window: notifications with action buttons, dialogs,
							// toasts, clipboard, vibration, speech. A script that can ask
							// a question and be answered is a different kind of tool.
							label: 'Talking Back to Android',
							slug: 'bridge/talking-back',
						},
					],
				},
				{
					...
					items: [
						{
							// AFTER the three API lessons, not before: a first script is
							// far more convincing when it composes commands the learner
							// already ran by hand and already wants to stop retyping.
							// Carries LiveSandbox — see SANDBOX_PATH at the top of this
							// file, and move that constant if this slug ever changes.
							label: 'From Commands to Scripts',
							slug: 'automation/shell-scripts',
							badge: { text: 'Interactive', variant: 'tip' },
						},
						{
							// Requires a script to schedule, so it cannot precede the
							// lesson above. Covers cron AND termux-job-scheduler, plus the
							// thing that makes mobile different from a server: Doze, the
							// phantom process killer, and wakelocks.
							label: 'Making It Run Itself',
							slug: 'automation/scheduling',
						},
					],
				},
				{
					label: 'Serving From Your Pocket',
					items: [
						{
							// Local first, deliberately: a learner who has not yet seen
							// their own site on another device over Wi-Fi cannot tell a
							// tunnel failure from a server failure in the next lesson.
							label: 'A Web Server in Your Pocket',
							slug: 'serving/local-server',
						},
						{
							// LAST teaching lesson, because it is the only one that makes
							// the device reachable by strangers. It needs every safety
							// habit the course has built by now.
							label: 'Opening a Door to the Internet',
							slug: 'serving/tunnels',
							badge: { text: 'Careful', variant: 'caution' },
						},
						// The terminus: gives the chain a destination and hands the
						// learner on to course three.
